...
$(function () {
    ...
    $('input[name="search"]').autocomplete({
        ...
        select: function (event, ui) {
            ...
            mov_stock_ajuste.items.productos.length=0;
            ui.item.cantidad = 1;//o podriamos poner directo en la vista
            console.log(mov_stock_ajuste.items);
            mov_stock_ajuste.add(ui.item);//se va agregar el producto y se va alistar automaticamente a travez de la funcion add
            $(this).val('');
        }
    });
    ...
    $('#tbProductos tbody')
        .on('click','a[rel="remove"]',function () {
            ...
            alert_action('Notificación', '¿Estas seguro de eliminar?',
                function () {
                    mov_stock_ajuste.items.productos.splice(tr.row,1);
                    mov_stock_ajuste.list();
                });
        }) 
        //cantidad
        ...
        var tr = tbProductos.cell($(this).closest('td, li')).index();
        mov_stock_ajuste.items.productos[tr.row].cantidad = cantidad;
      });
    ...
    $('form').on('submit', function (e) {
        ...
        if(mov_stock_ajuste.items.productos.length === 0){
            message_error('Debe cargar al menos un producto ');
            return false;
        }

        mov_stock_ajuste.items.fecha_mov = $('input[name="fecha_mov"]').val();
        mov_stock_ajuste.items.descripcion= $('input[name="descripcion"]').val();
        var parameters = new FormData();
        parameters.append('action', $('input[name="action"]').val());
        parameters.append('mov_stock_ajuste', JSON.stringify(mov_stock_ajuste.items));
